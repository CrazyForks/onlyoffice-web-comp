...
1. **Static assets**: Host the OnlyOffice SDK (including `web-apps/`, `sdkjs/`, `fonts/`, `x2t-1/`) under a public path, default `public/v9.3.0.24-1/`.
2. **Env var** (optional): `NEXT_PUBLIC_APP_ROOT=/v9.3.0.24-1` — must match `STATIC_RESOURCE.onlyoffice.root`.
...
构建时编辑 `public/v9.3.0.24-1/fonts/custom-fonts.json`（脚本源文件），运行：
