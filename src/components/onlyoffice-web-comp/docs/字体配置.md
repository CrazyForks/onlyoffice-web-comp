...
1. 将源字体编码为 `public/packages/onlyoffice/9.3.0/fonts/{id}`（**无扩展名**）。
...
| 路径 | 说明 |
|------|------|
| `public/packages/onlyoffice/9.3.0/fonts/ttf-to-catalog-font.mjs` | 与 SDK 同目录，部署时直接使用 |
| `src/components/onlyoffice-web-comp/scripts/fonts/ttf-to-catalog-font.mjs` | 组件库内副本，便于版本管理 |

将源字体放到脚本同目录（如 `1001.ttf`），或显式传入路径：

```bash
# 从同目录读取 1001.ttf → public/packages/onlyoffice/9.3.0/fonts/1001
...
```
public/packages/onlyoffice/9.3.0/fonts/1001
```
...
- `--out` / `--fonts-dir`：输出目录，默认 `public/packages/onlyoffice/9.3.0/fonts/`。
...
2. 将 catalog 线格式文件放到 `public/packages/onlyoffice/9.3.0/fonts/{索引号}`（无扩展名）。
